...
class User(Base):
    ...
    level = Column(Enum('A1', 'A2', 'B1', 'B2', 'C1',
                   'C2', name='level'), default='A1')

    users_words = relationship(
        "UserWord", back_populates="user", cascade="all, delete-orphan")
    non_parsed_word = relationship("NonParsedWord", back_populates="user")
    user_role = relationship(
        "UserRole", back_populates="user", uselist=False, cascade="all, delete")
...
class Role(Base):
    ...
    name = Column(String, Enum('Admin', 'Manager', 'User',
                  name='user_roles_enum'), unique=True)
...
class Word(Base):
    ...
    word_type_id = Column(Integer, ForeignKey(
        'word_types.id', ondelete='CASCADE'), nullable=False)
    english = Column(String, nullable=False)
    level = Column(String)
    __table_args__ = UniqueConstraint(
        'word', 'word_type_id', name='_unique_word'),

    word_type = relationship("WordType", back_populates="words")
    users_word = relationship(
        "UserWord", back_populates="word", cascade="all, delete")
    example = relationship("WordExample", back_populates="word",
                           uselist=False, cascade="all, delete")
    non_parsed_word = relationship(
        "NonParsedWord", back_populates="word", uselist=False, cascade="all, delete")
...
class Topic(Base):
    ...
    users_words_topics = relationship(
        "UserWordTopic", cascade="all, delete", back_populates="topic")
...
class WordExample(Base):
    ...
    word_id = Column(Integer, ForeignKey(
        'words.id', ondelete='CASCADE'), unique=True, nullable=False)
...
class UserWord(Base):
    ...
    user_word_topic = relationship(
        "UserWordTopic", cascade="all, delete-orphan", back_populates="user_word")
    custom_translation = relationship("UserWordTranslation", back_populates="user_word",
                                      uselist=False, cascade="all, delete")
    example = relationship(
        "UserWordExample", back_populates="user_word", uselist=False, cascade="all, delete")
    user_level = relationship(
        "UserWordLevel", cascade="all, delete", back_populates="user_word", uselist=False)
...
class UserWordTranslation(Base):
    ...
    id = Column(Integer, Sequence(
        'users_words_translations_id_seq'), primary_key=True)
    user_word_id = Column(Integer, ForeignKey(
        'users_words.id', ondelete='CASCADE'), nullable=False)
...
class UserWordExample(Base):
    ...
    id = Column(Integer, Sequence(
        'users_words_examples_id_seq'), primary_key=True)
    user_word_id = Column(Integer, ForeignKey(
        'users_words.id', ondelete='CASCADE'), unique=True, nullable=False)
    example = Column(String, nullable=False)
    translation = Column(String)

    user_word = relationship(
        "UserWord", back_populates="example", uselist=False)
...
class UserWordTopic(Base):
    ...
    id = Column(Integer, Sequence(
        'users_words_topic_id_seq'), primary_key=True)
    user_word_id = Column(Integer, ForeignKey(
        'users_words.id', ondelete='CASCADE'))
    ...
    __table_args__ = UniqueConstraint(
        'user_word_id', 'topic_id', name='_unique_user_word_topic'),
...
class NonParsedWord(Base):
    ...
    word_id = Column(Integer, ForeignKey(
        'words.id', ondelete='CASCADE'), unique=True)
    user_id = Column(Integer, ForeignKey('users.id', ondelete='CASCADE'))

    word = relationship(
        "Word", back_populates="non_parsed_word", cascade="all, delete")
    user = relationship(
        "User", back_populates="non_parsed_word", cascade="all, delete")
...
class UserWordLevel(Base):
    ...
    id = Column(Integer, Sequence(
        'users_words_levels_id_seq'), primary_key=True)
    user_word_id = Column(Integer, ForeignKey(
        'users_words.id', ondelete='CASCADE'), unique=True)
